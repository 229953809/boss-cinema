...
static void sendEndFileToJava(JNIEnv *env, mpv_event_end_file *event)
{
    ...
    env->CallStaticVoidMethod(mpv_MPVLib, mpv_MPVLib_eventEndFile_ii,
        (jint) reason, (jint) error);
}
...
void *event_thread(void *arg)
{
    ...
    while (1) {
        ...
        case MPV_EVENT_END_FILE:
            ALOGV("event: %s reason=%d error=%d\n", mpv_event_name(mp_event->event_id),
                mp_event->data ? ((mpv_event_end_file*)mp_event->data)->reason : -1,
                mp_event->data ? ((mpv_event_end_file*)mp_event->data)->error : 0);

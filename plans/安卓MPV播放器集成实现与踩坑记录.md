...
Exo 高性能缓冲会增加 max buffer、target buffer bytes 和带宽估算。MPV 当前分两层处理：

- libmpv 内存层：映射 demuxer cache / stream buffer / cache pause。
- HLS proxy 层：独立轻量分片缓存和预载，不复用 Exo `SimpleCache`。

不要把 Exo `PreCache`/`SimpleCache` 直接塞给 MPV。MPV proxy 的 cache key 必须包含 headers，并且要处理 Range、key、session、MIME 元信息和 seek 后预载。
...
### 17. 拼接源不能直接把 Exo URL 丢给 MPV

问题：

Exo 支持 `url|||duration***url|||duration`，由 `ConcatenatingMediaSource2` 拆成多个 MediaSource。MPV 如果直接 `loadfile` 这个字符串，会把它当成一个坏 URL，表现为加载失败或黑屏超时。

处理：

MPV 侧在 `playableUri()` 检测同款拼接格式，转换为 inline EDL：

- `edl://file=%<len>%<url>,length=<seconds>;...`
- duration 按 Exo 的 durationUs 转秒。
- URL 使用 EDL 的 `%<byte_length>%<value>` 写法，避免逗号、分号等字符破坏 EDL。

对应 Exo：

Exo 的 `MediaSourceFactory.createConcatenatingMediaSource()` 仍是成熟参考。MPV 不要复刻 MediaSource，而是用 mpv 原生 EDL 能力承接。

### 18. HLS proxy 缓存不要和 Exo SimpleCache 混用

问题：

Exo `SimpleCache` 服务的是 Media3 DataSource/CacheKey 体系；MPV 走本地 HTTP proxy，直接混用 Exo cache 容易出现 header、Range、key、session 元数据不一致，尤其是 AES key、fMP4 init segment 和 BYTERANGE。

处理：

MPV HLS proxy 使用独立轻量缓存：

- cache key = `md5(url + "\n" + headers)`，避免不同 header/session 串缓存。
- 只缓存非 playlist、非 Range、HTTP 200、非 PNG 前缀清洗、长度已知且不超过容量的二进制响应。
- 保存 MIME 元信息，缓存命中时可返回正确 `Content-Type`。
- 缓存命中支持 `Range` / `206` / `Content-Range` / `Accept-Ranges`。
- 临时文件使用唯一文件名，避免播放线程和预载线程同时写同一个 `.tmp`。
- 容量按播放缓存和预载容量共同限制，LRU 删除 `.bin` 时同步删除 `.meta`。

对应 Exo：

Exo 的 `PreCache` 负责 seek 后重新预缓存；MPV 代理通过记录 VOD `#EXTINF` 时间轴，在初始 playlist 和 seek 后按当前位置预载后续分片。

### 19. HLS live 恢复只能在 MPV 内核内做

问题：

Exo 遇到 `ERROR_CODE_BEHIND_LIVE_WINDOW` 会 `seekToDefaultPosition()`。MPV 没有等价 Java error code，直播窗口过期常表现为分片 404/410、playlist 滞后、加载失败或无音视频。

处理：

`MpvHlsProxy` 记录每个 session 的 playlist 类型、playlist/item 请求数和最近 HTTP 状态。`MpvPlayerEngine` 只在以下条件同时满足时恢复：

- 当前是 HLS。
- 代理没有识别到 VOD `#EXT-X-ENDLIST`。
- 最近有 HTTP 错误，或 MPV 报加载/HLS/无音视频类错误。
- 当前媒体最多重载两次。

恢复动作是在 MPV 内核内重新加载当前 `MediaItem`，相当于回到 live edge；不切 Exo，不自动切软解。VOD 404 仍然 fatal。

### 20. 系统 caption 样式需要映射到 libass

问题：

Exo 字幕样式来自 `SubtitleView` 和 Android `CaptioningManager`。MPV 走 libass，不会读取 Exo `SubtitleView`，只改 `SubtitleView` 对 MPV 无效。
...
- 字幕位置：`sub-pos`
- 前景色：`sub-color`
- 描边色：`sub-border-color`
- 阴影色：`sub-shadow-color`
- 背景色：`sub-back-color`
- 描边大小：`sub-border-size`
- 阴影偏移：`sub-shadow-offset`

对应 Exo：

Exo 仍由 `ExoUtil.setPlayerView()` 操作 `SubtitleView`。这是同一 UI 的不同底层实现，不要试图让 MPV 使用 Exo 字幕渲染组件。

### 21. native END_FILE reason/error 当前受源码阻塞

问题：

Java 只收到 `MPV_EVENT_END_FILE` 的 event id，没有收到 `mpv_event_end_file.reason` 和 `error`。没有 native payload 时，黑屏/超时只能靠日志和属性推断。

处理：

当前仓库只有 `MPVLib.java` 和打包好的 `libplayer.so`，没有 JNI/C 源码。Java 侧已补诊断和错误分类，但不能真正读取 native reason/error。后续必须补入 `libplayer.so` 的 JNI 源码或明确 native 库来源，再把 `mpv_event_end_file` payload 传到 Java。

对应 Exo：

Exo 的 `PlaybackException.errorCode` 和 `ErrorMsgProvider` 可以直接区分加载、解析、解码、DRM 等错误。MPV 在 native payload 接入前只能做“尽量准确的日志推断”，不能把推断当成完整错误码体系。
